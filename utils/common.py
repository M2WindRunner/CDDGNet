...
def plot_results(result_paths, save_dir=None, names=None):
    ...
    for result_path in result_paths:
        ...
        data = np.genfromtxt(result_path, skip_header=1, invalid_raise=False)
        if data.size == 0:
            continue
        if data.ndim == 1:
        ...
        if data.shape[1] < 10:
            continue
        valid_rows = np.all(np.isfinite(data[:, 1:10]), axis=1)
...
def check_dirs(run_name=""):
    ...
    if run_name:
        ...
        if any(character in invalid_characters for character in run_name):
            raise ValueError("run_name contains invalid characters")
        new_file_name = run_name
        if os.path.exists(os.path.join('./runs/train', new_file_name)):
            raise FileExistsError("run name exists: {}".format(new_file_name))
    else:
        file_names = os.listdir('./runs/train')
        file_names = [int(i) for i in file_names if i.isdigit()] + [0]
...
def fill_hole(cd_pred, device):
    ...
    im_out = torch.tensor(im_out.astype(np.int64)).cuda()
    return im_out
...
class SaveResult:
    ...
    def prepare(self):
        with open(self.result_save_path, "w")as f:
            headers = [
                'epoch', 'lr', 'P', 'R', 'F1', 'mIOU', 'OA', 'best_metric', 'train_loss', 'val_loss', 'de_loss'
            ]
    ...
    def show(self, p, r, f1, miou, oa,
        ...
        with open(self.result_save_path, "a")as f:
            row = [
                ...
                f"{float(r.mean()):.6f}",
                f"{float(f1.mean()):.6f}",
                f"{float(miou.mean()):.6f}",
                f"{float(oa.mean()):.6f}",
                f"{float(best_metric):.6f}",
                f"{float(train_avg_loss):.6f}",
                f"{float(val_avg_loss):.6f}",
                f"{float(exp_loss):.6f}",
            ]
            f.write("\t".join(row) + "\n")
